...
import { allCourses, courseCategories } from "@/components/customer/courses/course-data"

const levelFilters = ["সব", "শিক্ষার্থী", "মধ্যবর্তী", "অ্যাডভান্সড"]
const sortOptions = [
...
export default function CoursesPage() {
  ...
  const filteredCourses = allCourses
    .filter((course) => {
      ...
      const matchesLevel = selectedLevel === "সব" || course.level === selectedLevel
  ...
  const hasActiveFilters = selectedCategory !== "সব" || selectedLevel !== "সব" || search.trim() !== ""
    ...
    <main>
      ...
      <section className="bg-gradient-to-br from-[#0f172a] via-[#1e1b4b] to-[#0f172a] py-12 text-white">
        <div className="mx-auto w-full max-w-screen-2xl px-4 lg:px-6">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-300">আমাদের কোর্সসমূহ</p>
          ...
          <p className="mt-2 max-w-lg text-sm leading-relaxed text-slate-300">
            বিশেষজ্ঞ শিক্ষকদের কাছ থেকে শিখুন। নিজের গতিতে, নিজের সুবিধামতো সময়ে।
          </p>
      ...
      <div className="mx-auto w-full max-w-screen-2xl px-4 py-8 lg:px-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-slate-500">
              <span className="font-black text-slate-900">{filteredCourses.length}</span> টি কোর্স পাওয়া গেছে
            </span>
            {hasActiveFilters && (
              <button
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-[13px] font-bold ring-1 transition-all ${
                showFilters ? "bg-indigo-600 text-white ring-indigo-600" : "bg-white text-slate-700 ring-slate-200 hover:bg-slate-50"
              }`}
            >
        ...
        {showFilters && (
          <div className="mt-3 flex flex-wrap items-center gap-2 rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
            <span className="text-[12px] font-bold uppercase tracking-wider text-slate-400">স্তর:</span>
        ...
        {filteredCourses.length > 0 ? (
          ...
          <div className="mt-16 flex flex-col items-center gap-4 text-center">
            ...
            <h3 className="text-lg font-black text-slate-900">কোনো কোর্স পাওয়া যায়নি</h3>
            <p className="max-w-sm text-sm text-slate-500">
              আপনার সার্চ বা ফিল্টার পরিবর্তন করুন। অন্য কিওয়ার্ড দিয়ে চেষ্টা করুন।
            </p>
            <button
              onClick={clearFilters}
