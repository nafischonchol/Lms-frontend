...
import { TeachersListFilters } from "@/components/admin/teachers/TeachersListFilters";
import { TeacherStatusToggleButton } from "@/components/admin/teachers/TeacherStatusToggleButton";
...
export default async function TeachersListPage({
  ...
  const teachersResult = await getTeachersList({
    page: currentPage,
    per_page: 20,
    search: searchValue || undefined,
  });

  const { items: teachers, pagination } = teachersResult;
  ...
  const getPageHref = (pageNumber: number) => {
    ...
    return `/admin/teachers/list?${query.toString()}`;
  };
  ...
  return (
    <div className="w-full space-y-6 px-3 py-4 md:px-4 lg:px-5">
      <PageHeader
        title=""
        breadcrumbs={[{ label: "Home", href: "/admin" }, { label: "Teachers" }]}
      />
                  ...
                  <tr key={teacher.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <TeacherStatusToggleButton teacherId={teacher.id} isActive={teacher.is_active} />
